...
function App() {
  ...
  const [preferences, setPreferences] = useState({ theme: 'light', fontSize: 'medium', fontType: 'default', });
  ...
  // Mode debug: paksa tampilan loading via URL ?debug_loading=true
useEffect(() => {
  // Hanya aktif di development
  if (!import.meta.env.DEV) return;

  const params = new URLSearchParams(window.location.search);
  const debugLoading = params.get('debug_loading');
  ...
  const handlePreferencesChange = (newPrefs, newUserId) => {
    //
    console.log('Prefs dari backend:', newPrefs);
    //
...
const toggleModal = () => {
  ...
  const normalizedFontType = preferences.fontType || preferences.fontStyle || 'default';
  const normalizedFontSize = preferences.fontSize || preferences.font_size || 'medium';
  const fontSizeClass =
  {
    small: 'text-sm',    // 🔤
    medium: 'text-base', // 📐
    large: 'text-lg',    // 📏
  }[normalizedFontSize] || 'text-base';
  const labelTextClass =
  {
    small: 'text-[10px]',
    medium: 'text-xs',
    large: 'text-sm',
  }[normalizedFontSize] || 'text-xs';
  const fontTypeClass =
  normalizedFontType === 'serif'
  ? 'font-serif'
  : normalizedFontType === 'dyslexic' || normalizedFontType === 'open-dyslexic'
  ? 'font-dyslexic'
  : 'font-sans';
  ...
  w-full h-screen overflow-hidden flex flex-col transition-colors duration-300
  ${themeClass} ${fontSizeClass} ${fontTypeClass}
  ${isDark
...
`;
  ...
  <div className="flex items-center justify-center p-3 lg:p-8 animate-fade-in h-full overflow-y-auto">
    ...
    {quizData.questions[currentQuestionIndex] ? (
        ...
        <main className="flex-1 min-h-0 relative overflow-hidden flex flex-col">
          <div className="flex-1 p-0 overflow-hidden">
            <Question
              ...
              isDark={isDark}
              userId={userId}        // Mengirimkan userId
              difficulty={difficulty} // Mengirimkan difficulty
              currentQuestionIndex={currentQuestionIndex} // Mengirimkan currentQuestionIndex
              quizData={quizData} // Mengirimkan quizData ke Question
              handleNextQuestion={handleNextQuestion} // Passing the next button handler
            />
          ...
          <div className={`w-full max-w-5xl shadow-2xl rounded-xl overflow-hidden ${isDark ? 'bg-gray-800' : 'bg-white'}`}>
            <div className="flex flex-col-reverse lg:flex-row">
              <div className="w-full lg:w-3/5 p-6 sm:p-12 flex flex-col justify-center">
                ...
                <span className={`
                px-2 py-1 rounded-full ${labelTextClass} font-bold border flex items-center gap-2
              ...
              `}>
                ...
                <p className={`mb-6 sm:mb-8 ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>
                  Laporan Belajar Cerdas & Adaptif.
                </p>
                
                <div className="w-full mb-6 text-left">
                <p className={`${labelTextClass} font-bold mb-2 uppercase opacity-70 flex items-center`}>
                ...
                <button
                  ...
                  className={`w-full py-3 sm:py-4 px-6 rounded-lg font-bold text-white shadow-lg transition-all transform hover:-translate-y-1
      ...
      {/* Modal Pop-up untuk History Chart */}
      {showModal && (
